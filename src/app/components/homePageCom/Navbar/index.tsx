...
const Navbar = () => {
  ...
  return (
    <div className="relative">
            ...
            <div className="flex lg:items-center lg:mr-[55px] sm:flex-col ">
              ...
              {tvlVariation24h ? (
                <div className="flex items-center">
                  ...
                  {+tvlVariation24h > 0 ? (
                    <span className={`flex items-center text-xs text-green-10 sm:ml-[4px]`}>
                      (<NavBarUpArrowIcon className="mr-0.5"></NavBarUpArrowIcon>
                      {displayTvlVariation24h()})
                    </span>
                  ) : (
                    <span className={`flex items-center text-xs text-error sm:ml-[4px]`}>
      ...
      {float && <div
        className={float ? 'fixed left-0 top-0 w-full bg-dark-320 z-50' : ''}
        style={{
      ...
      >
        <div className="relative z-50 flex items-center justify-between mx-auto lg:w-full lg:px-20 sm:w-full md:w-full h-[44px] sm:px-6 md:px-6">
        <Logo className="sm:hidden" />
        <RefMobileLogo className="lg:hidden" />
          <LinkMobile className={float ? 'sm:hidden md:hidden' : 'sm:-mr-4'} />
        </div>
      </div>}
    </div>
  );
