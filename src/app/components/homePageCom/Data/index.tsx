...
import { useTokenList, Token, useTokenBaeInfoList, TokenBaseInfo } from '@/app/hooks/home';
import icons from '@/utils/metadata';
import { TopTokenIcon } from '../../layoutIcon/Icon';
import { toInternationalCurrencySystem } from '@/utils/numbers';
import styles from './index.module.css'
import { BannerArrowIcon,InCrease,DeCrease } from '../../layoutIcon/Icon';
import { MobileGrid } from '../../icon/SDKIntro';
    ...
    <div className="relative sm:mt-[70px]">
      <div className="relative z-10 flex flex-col items-center justify-center mx-auto lg:mt-[120px]">
        ...
        <div className="text-[54px] font-bold w-full flex items-center mt-[25px] sm:hidden">
          <span className={styles.gthemost}>The most</span>
          <span className="text-green-10 mx-[16px]">popular</span>
          <span className={styles.gtoken}>token</span>
        </div>

        <div className="text-[40px] font-bold w-full fccc mt-[40px] lg:hidden relative">
        <MobileGrid className="absolute top-[-140px]" />
          <span className={styles.gthemost}>The most</span>
          <div>
            <span className="text-green-10 mx-[16px]">popular</span>
            <span className={styles.gtoken}>token</span>
...
function TokenBox({ token, symbolMap }: { token: Token; symbolMap: any }) {
  const { symbol, icon, amount, tvl, volume24h, price,float_ratio } = processToken(token);
  const k = float_ratio.indexOf('-') > -1 
  return (
    <>
      <div className="rounded-[10px] p-5 text-white bg-dark-330 hover:opacity-85 sm:hidden">
        <div className="flex items-center">
          ...
          <div className="flex flex-col ml-2.5">
            ...
            <div className='frcc'>
              <span className="text-xl text-white font-medium">${Number(price).toFixed(4)}</span>
              <span className={`${!k ? 'text-green-10' : 'text-error'} text-base frcc ml-[4px]`}>
                {!k ? <InCrease/> : <DeCrease/>}&nbsp;
                {!k ? float_ratio : float_ratio.split('-')[1]}%
              </span>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div className="relative flex items-center justify-between w-full mt-[6px]">
          ...
          <div className='frcc'>
          <span className="text-lg text-white font-medium">${Number(price).toFixed(4)}</span>
